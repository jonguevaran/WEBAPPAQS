...
fs.writeFileSync('DataAlmacenes.json', JSON.stringify(newData, null, 2));

let html = fs.readFileSync('DelegacionAlmacenes.html', 'utf8');

const fetchTransformLogic = `
...
html = html.replace('dbAquaservice = await response.json();', fetchTransformLogic);
fs.writeFileSync('DelegacionAlmacenes.html', html);
console.log('Successfully updated DataAlmacenes.json and DelegacionAlmacenes.html fetch logic.');
